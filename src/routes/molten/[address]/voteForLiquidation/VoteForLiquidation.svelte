<script lang="ts">
	...
	import { voteForLiquidation } from '$lib/adapters';
	...
	const inputs = moltenFundingInterface.getFunction('voteForLiquidation').inputs;
	let form: Form,
		error: string,
	...
	const submit = async (e: CustomEvent<SubmitData>) => {
		...
		try {
			try {
				tx = await voteForLiquidation($signer, $moltenFundingData);
			} finally {
				if (tx) {
					notifications = [...notifications, '⏱ Awaiting vote transaction to validate…'];
					$moltenStateUpdates = [...$moltenStateUpdates, await tx.wait()];
					notifications = [...notifications, `✅ Voted!`];
				}
			}
		} catch (err) {
...
{#if $moltenFundingData && !$moltenFundingData._votedForLiquidation}
	<Form {formMeta} on:submit={submit} bind:this={form}>
		<button type="submit" disabled={!$signer || lock}>Vote</button>
		{#if error}
			<Error message={error} />
		{/if}
